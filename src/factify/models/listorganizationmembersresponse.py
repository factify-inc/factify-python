"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .organizationmember import OrganizationMember, OrganizationMemberTypedDict
from .pagination import Pagination, PaginationTypedDict
from factify.types import BaseModel, UNSET_SENTINEL
from pydantic import model_serializer
from typing import List, Optional
from typing_extensions import NotRequired, TypedDict


class ListOrganizationMembersResponseTypedDict(TypedDict):
    r"""ListOrganizationMembersResponse contains a page of organization members."""

    pagination: PaginationTypedDict
    r"""Pagination contains cursor-based pagination metadata.
    Follows Google AIP-158 for pagination field naming.
    """
    items: NotRequired[List[OrganizationMemberTypedDict]]
    r"""List of organization members."""


class ListOrganizationMembersResponse(BaseModel):
    r"""ListOrganizationMembersResponse contains a page of organization members."""

    pagination: Pagination
    r"""Pagination contains cursor-based pagination metadata.
    Follows Google AIP-158 for pagination field naming.
    """

    items: Optional[List[OrganizationMember]] = None
    r"""List of organization members."""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["items"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
