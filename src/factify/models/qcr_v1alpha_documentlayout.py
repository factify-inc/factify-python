"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .qcr_v1alpha_layoutbinding import (
    QcrV1alphaLayoutBinding,
    QcrV1alphaLayoutBindingTypedDict,
)
from .qcr_v1alpha_layoutdefaults import (
    QcrV1alphaLayoutDefaults,
    QcrV1alphaLayoutDefaultsTypedDict,
)
from .qcr_v1alpha_pagegeometry import (
    QcrV1alphaPageGeometry,
    QcrV1alphaPageGeometryTypedDict,
)
from .qcr_v1alpha_sourcereference import (
    QcrV1alphaSourceReference,
    QcrV1alphaSourceReferenceTypedDict,
)
from datetime import datetime
from factify.types import BaseModel, UNSET_SENTINEL
from pydantic import model_serializer
from typing import List, Optional
from typing_extensions import NotRequired, TypedDict


class QcrV1alphaDocumentLayoutTypedDict(TypedDict):
    r"""Top-level layout structure, joined to a DocumentRecord by record_id.
    Used with the Block-based record representation (DocumentRecord.blocks).
    """

    bindings: NotRequired[List[QcrV1alphaLayoutBindingTypedDict]]
    r"""sparse element → layout entries"""
    defaults: NotRequired[QcrV1alphaLayoutDefaultsTypedDict]
    r"""Document-wide layout defaults."""
    extracted_at: NotRequired[datetime]
    pages: NotRequired[List[QcrV1alphaPageGeometryTypedDict]]
    r"""page-level geometry (all pages)"""
    record_id: NotRequired[str]
    r"""references DocumentRecord.id"""
    source: NotRequired[QcrV1alphaSourceReferenceTypedDict]
    r"""SourceReference links a record back to the source it was extracted from.

    Three typed fields identify WHAT, WHERE, and WHEN. Everything else —
    format, content hash, storage location, checksums, external URLs — goes
    in the attributes bag, because the relevant metadata varies by source type.

    Examples by source type:

    File upload:
    source_id  = \"intg_01j8k...\"                    (upload channel instance)
    remote_id  = \"req_01h2xcejqtf2nbrexx3vqjhp41\"   (upload request ID)
    attributes = {format: \"pdf\", location: \"uploads/.../original.pdf\",
    content_hash: \"a1b2c3...\", storage_etag: \"\\"abc\\"\"}

    Gmail integration:
    source_id  = \"intg_01k9m...\"                    (Gmail connector instance)
    remote_id  = \"18abc123def\"                       (Gmail message ID)
    attributes = {format: \"eml\", thread_id: \"...\",
    external_url: \"https://mail.google.com/...\"}

    Confluence sync:
    source_id  = \"intg_01p3q...\"                    (Confluence connector instance)
    remote_id  = \"12345678\"                          (Confluence page ID)
    attributes = {format: \"html\", space_key: \"ENG\",
    external_url: \"https://acme.atlassian.net/wiki/...\"}
    """


class QcrV1alphaDocumentLayout(BaseModel):
    r"""Top-level layout structure, joined to a DocumentRecord by record_id.
    Used with the Block-based record representation (DocumentRecord.blocks).
    """

    bindings: Optional[List[QcrV1alphaLayoutBinding]] = None
    r"""sparse element → layout entries"""

    defaults: Optional[QcrV1alphaLayoutDefaults] = None
    r"""Document-wide layout defaults."""

    extracted_at: Optional[datetime] = None

    pages: Optional[List[QcrV1alphaPageGeometry]] = None
    r"""page-level geometry (all pages)"""

    record_id: Optional[str] = None
    r"""references DocumentRecord.id"""

    source: Optional[QcrV1alphaSourceReference] = None
    r"""SourceReference links a record back to the source it was extracted from.

    Three typed fields identify WHAT, WHERE, and WHEN. Everything else —
    format, content hash, storage location, checksums, external URLs — goes
    in the attributes bag, because the relevant metadata varies by source type.

    Examples by source type:

    File upload:
    source_id  = \"intg_01j8k...\"                    (upload channel instance)
    remote_id  = \"req_01h2xcejqtf2nbrexx3vqjhp41\"   (upload request ID)
    attributes = {format: \"pdf\", location: \"uploads/.../original.pdf\",
    content_hash: \"a1b2c3...\", storage_etag: \"\\"abc\\"\"}

    Gmail integration:
    source_id  = \"intg_01k9m...\"                    (Gmail connector instance)
    remote_id  = \"18abc123def\"                       (Gmail message ID)
    attributes = {format: \"eml\", thread_id: \"...\",
    external_url: \"https://mail.google.com/...\"}

    Confluence sync:
    source_id  = \"intg_01p3q...\"                    (Confluence connector instance)
    remote_id  = \"12345678\"                          (Confluence page ID)
    attributes = {format: \"html\", space_key: \"ENG\",
    external_url: \"https://acme.atlassian.net/wiki/...\"}
    """

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(
            ["bindings", "defaults", "extracted_at", "pages", "record_id", "source"]
        )
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
